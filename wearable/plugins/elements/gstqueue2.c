...
#endif
#ifdef GST_EXT_QUEUE_PULL_PATCH
#define DEFAULT_BUFFER_PAD_MIN_SIZE 50*1024
#endif
...
static gboolean
change_current_range(GstQueue2 * queue, GstQueue2Range *req_range)
{
  guint64 curr_level = 0;
  guint64 curr_min_level = 0;

  curr_level = queue->current->writing_pos - queue->current->max_reading_pos;

  /* FIXME, find a good threshold based on the buffer size, incoming rate and content bitrate. */
  curr_min_level = MIN((queue->max_level.bytes*0.05), DEFAULT_BUFFER_PAD_MIN_SIZE);
  ...
  if ((range = find_range (queue, offset))) {
    if (queue->current != range) {
      ...
      if ((QUEUE_IS_USING_RING_BUFFER (queue)) ||
          ...
          (change_current_range(queue, range)))
      {
